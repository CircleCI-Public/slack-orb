...
BuildMessageBody() {
    ...
    export CHANGE_LOG_TEXT=$(git log --pretty=format:"- %s (%an)%n" HEAD...production-v2 | sed "s/\'//g" | sed "s/\"//g" | sed 's/(#\([0-9]\{1,\}\))/[<https\:\/\/github.com\/stoplightio\/platform-internal\/pull\/\1|#\1>]/g' | head -c3000)
    # shellcheck disable=SC2155,SC2046
