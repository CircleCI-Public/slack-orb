...
BuildMessageBody() {
  ...
  if [ -n "${SLACK_PARAM_CUSTOM:-}" ]; then
    ...
    custom_template="$(printf '%s' "$CUSTOM_BODY_MODIFIED" | sed 's/\\/\\\\/g' | sed 's/"/\\"/g' | sed 's/`/\\`/g')"
    # Expand variables and run subshell commands in the custom template.
    message_body="$(eval printf '%s' \""$custom_template"\")"
  else
    ...
    # Evaluate the template name to get the template body.
    included_template="$(eval printf '%s' \""$TEMPLATE\"")"
    # Escape special characters in environment variables present in the template.
    SanitizeVars "$included_template"
    # Expand variables.
    message_body="$(circleci env subst "$included_template")"
  fi

  # Insert the default channel. THIS IS TEMPORARY
  message_body="$(printf '%s' "$message_body" | jq ". + {\"channel\": \"$SLACK_DEFAULT_CHANNEL\"}")"
